    ...
    if request.method == 'POST':
        ...
        if len(userId) == 0 or len(userPw) == 0:
            ...
            for row in rows:
                ...
                # else:
                #     ctypes.windll.user32.MessageBoxW(0, "아이디 또는 비밀번호가 틀렸습니다.", "알림", 0)
...
# 회원가입
@app.route('/modal.html/register', methods=['GET', 'POST'])
def modal():
    if request.method == 'POST':
        result = request.form
...
def electron():
    if "userId" in session:
        return render_template('electron_page.html')
    else:
        return jsonify("Who are you?")
...
def life():
    if "userId" in session:
        return render_template('life_page.html')
    else:
        return jsonify("Who are you?")
